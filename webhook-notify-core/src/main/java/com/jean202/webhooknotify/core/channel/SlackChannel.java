...
public final class SlackChannel implements NotifyChannel {
    ...
    public void send(NotifyMessage message) {
        ...
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            throw new IllegalStateException(
                "Slack webhook request failed with status " + response.statusCode() + ": " + response.body()
            );
        }
    }

    private static String toPayload(NotifyMessage message) {
        return "{\"text\":\"" + escapeJson(renderText(message)) + "\"}";
    }

    private static String renderText(NotifyMessage message) {
        String title = message.title().trim();
        String body = message.body().trim();

        if (title.isEmpty() && body.isEmpty()) {
            throw new IllegalArgumentException("Slack notification text must not be blank");
        }
        if (title.isEmpty()) {
            return body;
        }
